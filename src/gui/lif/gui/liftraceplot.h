#ifndef LIFTRACEPLOT_H
#define LIFTRACEPLOT_H

#include <gui/plot/zoompanplot.h>
#include <memory>

#include <data/lif/lifconfig.h>
#include <data/lif/liftrace.h>

class BlackchirpEvenSpacedCurve;
class QwtPlotZoneItem;
class QwtPlotTextLabel;

namespace BC::Key {
inline constexpr QLatin1StringView lifTracePlot{"lifTracePlot"};
inline constexpr QLatin1StringView lifCurve{"lifCurve"};
inline constexpr QLatin1StringView refCurve{"lifRefCurve"};
}

class LifTracePlot : public ZoomPanPlot
{
    Q_OBJECT
public:
    LifTracePlot(QWidget *parent = nullptr);
    ~LifTracePlot();

signals:
    void integralUpdate(double);
    void lifGateUpdated(int,int);
    void refGateUpdated(int,int);

public slots:
    void setLifGateStart(int n);
    void setLifGateEnd(int n);
    void setRefGateStart(int n);
    void setRefGateEnd(int n);
    void setNumAverages(int n);
    void setAllProcSettings(const LifTrace::LifProcSettings &s);
    void processTrace(const LifTrace t);
    void setTrace(const LifTrace t);
    void checkColors();
    void reset();
    void setIntegralText(double d);

    void clearPlot();


private:
    std::unique_ptr<BlackchirpEvenSpacedCurve> p_lif, p_ref;
    std::unique_ptr<QwtPlotZoneItem> p_lifZone, p_refZone;
    std::unique_ptr<QwtPlotTextLabel> p_integralLabel;
    LifTrace d_currentTrace;
    int d_numAverages;
    bool d_resetNext;
    LifTrace::LifProcSettings d_procSettings;

    void updateLifZone();
    void updateRefZone();


    // ZoomPanPlot interface
protected:
    virtual void replot() override;

    // QWidget interface
public:
    QSize sizeHint() const override;
};

#endif // LIFTRACEPLOT_H
