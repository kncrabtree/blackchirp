...
bool AWG5204::testConnection()
{
    ...
    if(!resp.startsWith(QByteArray("TEKTRONIX,AWG5202")))
    {
...
QString AWG5204::getWaveformKey(const ChirpConfig cc)
{
...
QString AWG5204::writeWaveform(const ChirpConfig cc)
{
    ...
    currentChunk = 0;
    chunks = static_cast<int>(ceil(static_cast<double>(packedMarkers.size())/static_cast<double>(chunkSize)));
    ...
    while(currentChunk < chunks)
    {
        ...
        int endIndex = qMin((currentChunk+1)*chunkSize,packedMarkers.size());
        int numPnts = endIndex - startIndex;
