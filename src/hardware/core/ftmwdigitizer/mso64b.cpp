...
REGISTER_HARDWARE_SETTINGS(MSO64B,
    ...
    {maxRecordLength,    "Max Record Length",   "Maximum record length in samples",
     62500000, 0, QVariant{}, HwSettingPriority::Optional},
    ...
    {bandwidth,          "Bandwidth (MHz)",     "Analog bandwidth",
     2500.0, QVariant{}, QVariant{}, HwSettingPriority::Important}
)
REGISTER_HARDWARE_ARRAY(MSO64B, sampleRates,
    "Sample Rates", "Available digitizer sample rates",
    HwSettingPriority::Important)
REGISTER_HARDWARE_ARRAY_ENTRY(MSO64B, sampleRates,
...
bool MSO64B::testConnection()
{
    ...
    if(!resp.startsWith(QByteArray("TEKTRONIX,MSO64B")))
    {
        if(resp.length() > 50)
            resp = resp.mid(0,50);
...
void MSO64B::initialize()
{
    p_scopeTimeout = new QTimer(this);

    p_socket = p_comm->device<QTcpSocket>();
    connect(p_socket,static_cast<void (QTcpSocket::*)(QAbstractSocket::SocketError)>(&QTcpSocket::errorOccurred),this,&MSO64B::socketError);
    p_socket->setSocketOption(QAbstractSocket::LowDelayOption,1);
    p_socket->setSocketOption(QAbstractSocket::KeepAliveOption,1);
}

bool MSO64B::prepareForExperiment(Experiment &exp)
{
    ...
    else
    {
        ...
        resp = p_comm->queryCmd(QString(":HORIZONTAL:FASTFRAME:STATE ON;:HORIZONTAL:FASTFRAME:STATE?\n"));
        if(!resp.isEmpty())
        {
            bool ok = false;
            bool ffState = (bool)resp.trimmed().toInt(&ok);
    ...
    if(config.d_triggerChannel > 0)
    {
        ...
        resp = scopeQueryCmd(QString(":TRIGGER:A:EDGE:SOURCE %1;COUPLING DC;SLOPE %2;:TRIGGER:A:LEVEL:%1 %3;:TRIGGER:A:EDGE:SOURCE?;SLOPE?\n")
                                 .arg(trigCh).arg(slope).arg(config.d_triggerLevel,0,'f',3));
    }
    else
    {
        resp = scopeQueryCmd(QString(":TRIGGER:A:EDGE:SOURCE %1;COUPLING DC;SLOPE %2;:TRIGGER:AUXLEVEL %3;:TRIGGER:A:EDGE:SOURCE?;SLOPE?\n")
                                 .arg(trigCh).arg(slope).arg(config.d_triggerLevel,0,'f',3));
    }
    if(!resp.isEmpty())
    {
...
//                        .arg(config.d_numRecords).arg(resp.toInt()));
//            hwDebug(u"%1: Waveform contains the wrong number of frames. Target: %2, Actual: %3. Response = %4 (Hex: %5)"_s
//                        .arg(d_key).arg(config.d_numRecords).arg(resp.toInt()).arg(QString(resp), QString(resp.toHex())));
//            return false;
//        }
//    }
...
void MSO64B::beginAcquisition()
{
    if(d_enabledForExperiment)
    {
        ...
        connect(p_socket,&QTcpSocket::readyRead,this,&MSO64B::readWaveform,Qt::UniqueConnection);
        ...
        d_headerNumBytes = 0;
        d_waveformBytes = 0;
        connect(p_scopeTimeout,&QTimer::timeout,this,&MSO64B::wakeUp,Qt::UniqueConnection);
    }
}
...
void MSO64B::endAcquisition()
{
    ...
    if(d_enabledForExperiment)
    {
        ...
        disconnect(p_socket,&QTcpSocket::readyRead,this,&MSO64B::readWaveform);
        disconnect(p_scopeTimeout,&QTimer::timeout,this,&MSO64B::wakeUp);
...
void MSO64B::socketError(QAbstractSocket::SocketError e)
{
