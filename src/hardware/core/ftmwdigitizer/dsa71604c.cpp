...
REGISTER_HARDWARE_SETTINGS(Dsa71604c,
...
REGISTER_HARDWARE_ARRAY_ENTRY(Dsa71604c, sampleRates,
...
//    if(!d_comm->writeCmd(QString("CH%1:BANDWIDTH:ENHANCED OFF; CH%1:BANDWIDTH 1.6+10; COUPLING AC;OFFSET 0;SCALE %2\n").arg(config.fidChannel).arg(QString::number(config.vScale,'g',4))))
    ...
    //NOTE: this doesn't actually work-- just causes problems
    //Leaving the code commented for now as an example in case it is useful
    //in the future.
    // if(config.d_multiRecord || config.d_blockAverage)
    // {
    ...
    auto cfg = static_cast<FtmwDigitizerConfig*>(this);
    *cfg = config;
