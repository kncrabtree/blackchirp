 ...
 *
 * Adds a single hook on top of FileParser: the parse() method that
 * converts a recognized catalog file into a CatalogData value.
 * CatalogData contains the ordered list of transitions (frequency,
 * intensity, error, lower energy, quantum numbers, ...) plus
 * source-program metadata. The concrete subclasses shipped with
 * Blackchirp are SPCATParser and XIAMParser; a new format that exposes
 * the same transition shape (frequency, intensity, quantum numbers)
 * should derive from this class so that CatalogOverlay can consume it
...
public:
    ...
    /*!
     * \brief Parse a catalog file into a CatalogData value.
     *
     ...
     * are expected to check CatalogData::transitions().isEmpty() before
