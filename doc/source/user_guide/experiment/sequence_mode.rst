...
The sequence can be cancelled at any time by pressing the ``Abort`` button; pressing ``Abort`` during an active acquisition in the sequence stops that experiment immediately and does not start the next one.
