...
* ``lockExternal`` requests that the clock lock to an external 10 MHz reference at the start of an experiment. Implementations that cannot read back the lock state ignore the setting.
...
A virtual clock with up to six independently configurable outputs. Use this implementation for any fixed-frequency oscillator (e.g., a PLDRO) whose frequency Blackchirp does not tune.
...
The HP 83712B is a single-channel synthesizer tunable between 1 and 20,000 MHz. It connects through GPIB. The implementation has not been thoroughly tested.
